  ...
  formatDate(date, type, opts = {}) {
    ...
    switch(type) {
      ...
      case 'ymd.hm.short': {
        ...
        result = `${String(year).slice(2)}.${month.setPadding(2)}.${day.setPadding(2)} ${hour.setPadding(2)}:${minute.setPadding(2)}`
      ...
      case 'hms-lang-strong':  {
        ...
        let secondSep = '', minuteSep = ' : ', hourSep = ' : '
        ...
        if(type === 'hms-lang') {
          result = `${Number(date.toFormat('HH')).setPadding(2)}${hourSep}${date.get('minute').setPadding(2)}${minuteSep}${date.get('second').setPadding(2)}${secondSep}`
        }
