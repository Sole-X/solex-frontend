...
import {
  ...
  kip7ByteCode,
  kip17ByteCode,
} from 'caver-js/packages/caver-kct/src/kctHelper';
...
export default class WalletPlugin {
  ...
  async subscribeNewBlock() {
    ...
    const initBlock = await this.getRpcInstance()
      .getBlock('latest')
      .catch(() => {
  ...
  initBlockStatusWatcher() {
    // 사이트 초기화 이후 지연 상태가 되는 경우 대응
    let lastTryId = 0;
    let maxTryCount = 30;
...
web3-utils/src/index.js 의 toChecksumAddress가 this.utils에 포함이 되지 않아서 생기는 오류.
  ...
  */
  ...
  async getNftBalance(address) {
    ...
    const total = await _nft.methods
      .balanceOf(user)
      .call()
      .catch(() => {
        return 0;
    ...
    for (let i = 0; i < total; i++) {
      ...
      const tokenUri = await _nft.methods
        .tokenURI(tokenId)
        .call()
        .then((res) => {
          return res || {};
