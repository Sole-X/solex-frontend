<template>
  <div class="gen-modal dialog-modal" :class="getModalClass">
    ...
    <article v-if="data.event === 'NEGO_REJECT'" class="gen-modal__content about-my-offer__decline-reason">
      <textarea class="decline-nego__message-box" :value="data.info.myNegoForReject && data.info.myNegoForReject.declineReason" disabled />
    </article>
      ...
      getNoticeList() {
        ...
        if(event === 'NEGO_REJECT') {
          const rejectNegoDetail = _.find(RejectNegoTypes, row => {
            if (info.myNegoForReject)
              return row.id === info.myNegoForReject.declineType
          }) || { text: 'unknown reason' }
          ...
          return [
            {
               ...
               */
              text: this.$t('UserPage.AboutOfferRejectedDesc1') + ' ' + (typeof rejectNegoDetail.text === 'function' ? rejectNegoDetail.text($t, currencySymbol) : '')
            },
            {
    ...
    methods: {
      ...
      translateTimeFormat(time) {
        ...
        if (this.$t('General.Language') === 'KOR') {
          ...
          newTime += ` ${isAm ? '오전' : '오후'} ${isAm ? timeObj.getHours() : timeObj.getHours() - 12}:${timeObj.getMinutes()}에 입찰`;
        } else {
