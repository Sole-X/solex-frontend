  ...
  <section class="gen-modal decline-nego-modal">
    ...
    <article class="gen-modal__content gen-modal__sub-title">
      <div>
        <h4
          v-if="!isKlip"
          v-html="$t('Market.DeclineNegoWhy', { user: getMaskedAddress(data.negoInfo.accountAddress) })"
        />
        <h4
          v-else-if="isKlip"
          v-html="$t('Market.DeclineNegoKlip', { user: getMaskedAddress(data.negoInfo.accountAddress) })"
        />
      </div>
    </article>
    ...
    <template v-if="!isKlip">
      <article class="gen-modal__content decline-nego-modal__option">
        <ul>
          <li
            v-for="option in getDeclineOptions()"
            :key="option.type"
            :class="$bem('decline-nego-modal__radio', '', option.isSelected ? ['selected'] : '')"
    ...
    <article class="gen-modal__submit">
      <button
        :class="$bem('common-submit-button', '', [!enableSubmit && !isKlip ? 'disabled' : 'primary'])"
