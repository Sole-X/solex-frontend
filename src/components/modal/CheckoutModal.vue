    ...
    <div class="checkout-modal__checkout" v-if="step === 1">
      <article class="gen-modal__content gen-modal__sub-title">
        <div>
          ...
          <div class="gen-modal__desc">{{ this.data.info.isBuy ? $t('Market.ContinueCheckoutSellDescription') : $t('Market.ContinueCheckoutDescription')}}</div>
        </div>
      </article>
      ...
      <article class="gen-modal__content">
        <div class="checkout-modal__order">
          <div class="checkout-modal__order__head">
            <div class="checkout-modal__order__head__thumbnail" :style="$_AssetCardMixin_getImage(data.info)">
          ...
          <div class="checkout-modal__order__info">
            ...
            <div class="checkout-modal__order__info__row">
              ...
              <div class="checkout-modal__order__info__value">
                <p>
                  <strong>{{$bn.toMaxUnit(data.checkoutPrice, data.info.currencyInfo.decimal, 4) | addComma}} </strong> {{data.info.currencyInfo.symbol}}
                </p>
                <p class="checkout-modal__order__info__value__fiat">$ {{data.info.currencyInfo.toFiat($bn.toMaxUnit(data.checkoutPrice, data.info.currencyInfo.decimal)).dprec(4) | addComma}}</p>
              </div>
            </div>
      ...
      <article class="gen-modal__submit" @click="handleSubmit()">
        <button :class="$bem('common-submit-button', '', getButtonClass)">{{ this.data.info.isBuy ? $t('Market.DoCheckoutSell') : $t('Market.DoCheckout')}}</button>
      </article>
    </div>
  </section>
</template>
      ...
      isEnableCheckout() {
        if (this.data.info && this.data.info.isBuy) {
          ...
          const onTradeObj = _.find(onTrade, row => {
            if (this.$wallet.isSameAddress(row.tokenAddress, this.data.info.tokenAddress))
              return true;
          })
