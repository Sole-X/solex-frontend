<template>
  <section class="gen-modal dialog-modal share-content-modal">
    ...
    <article class="share-content__options">
      <div v-for="channel in getSupportChannel" :key="channel.type" 
      :class="channel.use ? '' : 'share-content__option__nouse'"
      class="share-content__option share-content__" @click="channel.onClick">
    ...
    <article class="share-content__url">
      ...
      <button v-clipboard:copy="data.url || ''" class="text-lightblack" v-clipboard:success="$_GlobalValueMixin_showCopyTooltip">
        {{$t('General.CopyUrl')}}
      </button>
    </article>
  </section>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getKakaotalkTitle() {
        const title = `[Sole-X] ${this.data.item.nftInfo ? this.data.item.nftInfo.name : ''} ${this.data.item.metadata ? this.data.item.metadata.name : ''}`;
        return title;
      },
