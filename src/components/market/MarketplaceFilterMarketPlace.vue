        ...
        >
          <div class="marketplace-filter-item-top">
            ...
            <div class="marketplace-filter-reset">
              <div
                class="marketplace-filter-reset-button"
                @click="resetClicked"
              >
              ...
              >
                ...
                >
                  ...
                  <div
                    class="item-value"
                    @click="
                      (event) => currenciesClicked(event, item.name, item)
                    "
                  >
                    {{ item.symbol }}
                  </div>
   ...
   */
       ...
       */
        ...
        blockchainClicked(event, value) {
          ...
          if (!event) {
            ...
            for (const elem of parentElem.children) {
              if (
                JSON.parse(elem.children[0].dataset.obj).value.includes(value)
              ) {
          ...
          const idx = this.filters.blockchain.findIndex(
            (i) => i.value === value
          );
          if (idx > -1) {
        ...
        categoriesClicked(event, value) {
          ...
          const idx = this.filters.categories.findIndex(
            (i) => i.value === value
          );
          if (idx > -1) {
        ...
        collectionsClicked(event, value, item) {
          ...
          const idx = this.filters.collections.findIndex(
            (i) => i.value === value
          );
        ...
        currenciesClicked(event, value, item) {
          ...
          const idx = this.filters.currencies.findIndex(
            (i) => i.value === value
          );
          if (idx > -1) {
                  ...
                  while (cur) {
                    if (JSON.parse(cur.dataset.obj).section === "collections") {
                      ...
                      if (
                        ...
                        JSON.parse(cur.dataset.obj).name === item.value
                      ) {
                        // price section
                        const child = cur.hasChildNodes()
                          ? cur.childNodes[0]
        ...
        currencyInPriceChanged(event) {
          ...
          for (const originFilterBy of this.filterby) {
            if (
              originFilterBy.section &&
              originFilterBy.section === "currencies"
            ) {
              for (const refFilterBy of this.$refs.filterby.children) {
                if (
                  refFilterBy.dataset.obj &&
                  JSON.parse(refFilterBy.dataset.obj).value ===
                    originFilterBy.value
                ) {
                  if (
          ...
          for (const refCurrencies of this.$refs.currencies.children) {
            if (refCurrencies.hasChildNodes()) {
              ...
              if (
                parentElem.hasChildNodes() &&
                parentElem.children.length >= 2
              ) {
                const elem = parentElem.children[1];
                if (
                  JSON.parse(parentElem.dataset.obj).symbol === symbolSelected
                ) {
                  elem.dispatchEvent(newEvent);
                }
              }
