    ...
    <article class="asset-item__profile__detail">
      <ul class="auction_detail_m">
        <li>
          <div>
            {{
              info.isEndedAuction ? $t('Market.FinalBid') : isNotBidder ? $t('Market.Bids0') : $t('Market.CurrentBid')
            }}
          </div>
          <div class="asset-item__profile__detail__bids">
            <div class="asset-item__profile__detail__bids__latest">
              <p>
                <strong class="text-black"
                  >{{ $bn.toMaxUnit(info.auction.currentPrice, info.currencyInfo.decimal, 4) | addComma }}
                </strong>
                {{ info.currencyInfo.symbol }}
              </p>
              ...
              <p class="text-gray">
                ...
                {{
                  info.currencyInfo
                    ? info.currencyInfo
                        .toFiat($bn.toMaxUnit(info.auction.currentPrice, info.currencyInfo.decimal))
                        .dprec(4)
                    : info.usdPrice | addComma
                }}
              </p>
            </div>
            ...
            <div class="asset-item__profile__detail__bids__others">
              <button class="asset-item__request__more" @click="handleOpenBids()">
                <img
                  :src="
                    $static.getFileURL(`img/icon/ic-message-${info.auction.bids.length > 0 ? 'black' : 'gray'}.svg`)
                  "
                />
        ...
        <li>
          ...
          <div class="asset-form__price">
            <div class="asset-form__price__timer">
              ...
              >{{ $t('Market.AuctionDay') }} : <strong>{{ remainPeriod.hours.setPadding(2) }}</strong
              >{{ $t('Market.AuctionHour') }} : <strong>{{ remainPeriod.minutes.setPadding(2) }}</strong
              >{{ $t('Market.AuctionMinute') }} : <strong>{{ remainPeriod.seconds.setPadding(2) }}</strong
              >{{ $t('Market.AuctionSecond') }}
            </div>

            <div class="asset-form__price__ended">
              {{ $t('Market.EndedAt') }} <span class="divider">|</span>
              {{ $date.formatDate(info.auction.endTime, 'fff') }}
            </div>
          </div>
        </li>
      </ul>
      ...
      <ul>
        <li class="asset-item__profile__detail__price">
          ...
          <div>
            ...
            <p>$ {{ !this.info.isEndedAuction ? getStraightPrice('usd') : getFinalPrice('usd') | addComma }}</p>
          </div>
        </li>
      </ul>
    </article>
...
export default {
  ...
  computed: {
    ...mapGetters(['getItemInfo', 'getSupportCurrency']),
  ...
  methods: {
    ...mapActions(['cancelAuction', 'closeAuction']),
    ...
    getStraightPrice(type) {
      if (type === 'token') {
        ...
        else return this.$bn.toMaxUnit(this.info.auction.currentPrice, this.info.currencyInfo.decimal, 4);
      } else if (type === 'usd') {
        if (this.info.auction && this.info.auction.type === 2)
          return this.info.currencyInfo
            .toFiat(this.$bn.toMaxUnit(this.info.auction.straightPrice, this.info.currencyInfo.decimal))
            .dprec(4);
        else
          return this.info.currencyInfo
            .toFiat(this.$bn.toMaxUnit(this.info.auction.currentPrice, this.info.currencyInfo.decimal))
            .dprec(4);
      }
    },
