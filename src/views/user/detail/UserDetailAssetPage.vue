  ...
  <section class="user-asset-page">
    ...
    <article class="user-collapsible user-asset__in-wallet">
      <ui-collapsible ref="inWallet">
        <user-collapsible-header slot="header" :title="$t('UserPage.ShowInWallet')" :total="getCount('inWalletItems')" />

        <section class="user-collapsible__content" ref="inWalletItems">
          <div class="user-asset__in-wallet__row" v-for="currency in getSupportCurrency" v-show="isAvailToken(currency)" :key="currency.symbol">
            ...
            <div class="user-asset__in-wallet__action">
              <button :class="$bem('common-submit-button', '', [(isAvailToken(currency) ? 'primary' : 'disabled')])" @click="handleDepositCurrency(currency)">
    ...
    <article class="user-collapsible user-asset__in-wallet">
      <ui-collapsible ref="inSolex">
        <user-collapsible-header slot="header" :title="$t('UserPage.ShowInReserve')" :total="getUserDeposited.tokens.length" />
        ...
        <section v-else class="user-collapsible__content">
          ...
          >
            <div class="user-asset__col__icon">
              <img :src="$static.getFileURL(`img/icon/ic-token-${(currency.tokenInfo.symbol || '').toLowerCase()}.svg`)" alt="eth" />
            </div>

            <div class="user-asset__col__name">
              <p><strong>{{currency.tokenInfo.symbol}}</strong></p>
              <p class="text-gray">{{currency.tokenInfo.name}}</p>
            </div>

            <div class="user-asset__in-wallet__link">
              <a :href="$wallet.getTokenUrl(currency.tokenInfo.addressToReserve, getChainInfo.chain)" target="_blank" rel="noopener noreferrer">
            ...
            <div class="user-asset__col__balance">
              ...
              <p class="text-gray">
                <strong>{{currency.tokenInfo.toFiat(currency.getDecAmount(currency.depositAmount)).dprec(4) | addComma}} USD</strong>
              </p>
            </div>

            <div class="user-asset__in-wallet__action">
              <button v-if="isAvailToken(currency)" :class="$bem('common-submit-button', '', ['primary'])" @click="handleWithdrawCurrency(currency)">
                {{$t('UserPage.WithdrawToMyWalletAsset')}}
              </button>
              <button v-else :class="$bem('common-submit-button', '', ['primary'])" @click="handleWithdrawCurrencyToOther(currency)">
    ...
    <article class="user-collapsible user-asset__in-order">
      <ui-collapsible ref="onSale">
        <user-collapsible-header slot="header" :title="$t('UserPage.ShowOnOffer')" :total="getOnSaleOrAuctionAssets.length" />
        ...
        <section class="user-collapsible__content">
          <div class="user-asset__in-order__row" v-for="(currency, i) in getOnSaleOrAuctionAssets" :key="currency.symbol">
            <div class="user-asset__col__icon">
              <img :src="$static.getFileURL(`img/icon/ic-token-${(currency.tokenInfo.symbol || '').toLowerCase()}.svg`)" alt="eth" />
            </div>
            ...
            <div :class="$bem('user-asset__col__balance', '', '')">
              ...
              <p class="text-gray">
                <strong>{{currency.tokenInfo.toFiat(currency.getDecAmount(currency.onOrderAmount)).dprec(4) | addComma}} USD</strong>
              </p>
            </div>
            ...
            <div class="user-asset__in-order__card-list">
              <section>
                <div
                    ...
                    :style="{'background-image': `url('${getImageUrl(order.nftInfo)}')`, 'background-size': 'contain', 'background-position': 'center center'}"
                >
                  <ui-tooltip class="user-asset__in-order__tooltip" position="top">
                    <p>
                      {{order.nftInfo.tokenId}} ({{order.nftInfo.tokenAddress.slice(0, 8)}})
                    </p>

                    <p>
                      <strong>
                        {{$bn.toMaxUnit(order.nftInfo.currentPrice, currency.tokenInfo.decimal, 4) | addComma}} {{currency.tokenInfo.symbol}}
                      </strong>
        ...
        <section v-else class="user-collapsible__content">
          <div class="user-asset__in-order__row" v-for="currency in getOnBuyAssets" :key="currency.symbol">
            ...
            <div :class="$bem('user-asset__col__balance', '', '')">
              ...
              <p class="text-gray">
                <strong>{{currency.tokenInfo.toFiat(currency.getDecAmount(currency.lockBuyAmount)).dprec(4) | addComma}} USD</strong>
              </p>
            </div>
            ...
            <div class="user-asset__in-order__card-list">
              <section>
                <div
                    ...
                    :style="order.desc ? {'background-image': `url('${getImageUrl(order)}')`, 'background-size': 'contain', 'background-position': 'center center'} :{}"
                >
                  <ui-tooltip class="user-asset__in-order__tooltip" position="top">
                    <p>
                      {{order.tokenId}} ({{order.tokenAddress.slice(0, 8)}})
                    </p>

                    <p>
                      <strong>
                        {{$bn.toMaxUnit(order.basePrice, currency.tokenInfo.decimal, 4) | addComma}} {{currency.tokenInfo.symbol}}
                      </strong>
  ...
  export default {
    ...
    methods: {
      ...
      getImageUrl(row) {
        ...
        if (row.nftInfoObject && row.nftInfoObject.desc && row.nftInfoObject.desc.image !== '') return row.nftInfoObject.desc.image;
        return '';
      },
