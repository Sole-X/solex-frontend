    ...
    <div v-if="isItem">
      <section class="staking-activity__table">
        <div class="staking-activity__table__head">
          <div :class="$bem('staking-activity__table__col', '', [col.type])" v-for="(col, i) in getTableCols" :key="i">
            <span class="text-gray">{{ translated(col.title) }}</span>
          </div>
        </div>

        <div v-if="getIsSetPager" class="staking-activity__table__body">
          <div v-for="(row, i) in getStakingItems" :key="i" class="staking-activity__table__row">
            ...
            <div :class="$bem('staking-activity__table__col', '', ['state'])">
              <button
                @click="
                  (event) =>
                    row.type === 2 && Date(row.due) <= Date()
                      ? claimButtonClicked(event, getCurrency(getTokenAddress))
                      : ''
                "
...
export default {
  ...
  methods: {
    ...mapActions(['claimUnstakingToken', 'setCurPage', 'setTotalPage', 'setStakingItems', 'setIsSetPager']),
    ...
    convertDate(date) {
      ...
      let retDate = `${monthStrs[d.month - 1]} ${d.day}, ${d.year}  ${d.hour > 9 ? d.hour : '0' + d.hour}:${
        d.minute > 9 ? d.minute : '0' + d.minute
      } GMT`;
      return retDate;
    },
