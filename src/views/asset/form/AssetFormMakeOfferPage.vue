    ...
    <section class="asset-form__body">
      ...
      <div class="asset-form__title">
        <h1>
          <strong>{{
            isWithBid
              ? $t('Market.MakeOfferTitleAuction')
              : !isSell
              ? $t('Market.MakeOfferTitleBuy')
              : $t('Market.MakeOfferTitleSell')
          }}</strong>
        </h1>
      ...
      <div v-if="isWithBid">
        <div class="asset-form__row">
          ...
          <div class="asset-form__currency">
            <button
              ...
              :class="
                $bem(
                  'asset-form__currency__option',
                  '',
                  isVisibleToken(currency)
                    ? usedCurrency.symbol === currency.symbol
                      ? ['selected']
                      : []
                    : ['disabled'],
                )
        ...
        <div class="asset-form__row">
          ...
          <div class="asset-form__make-offer-nego">
            <button
              :class="$bem('asset-form__make-offer-select__option', '', isInstantTrade === true ? ['selected'] : '')"
              @click="isInstantTrade = true"
            >
              {{ $t('General.Set') }}
            </button>
            <button
              :class="$bem('asset-form__make-offer-select__option', '', isInstantTrade === false ? ['selected'] : '')"
              @click="isInstantTrade = false"
            >
              {{ $t('General.None') }}
            </button>
          </div>
        ...
        <div class="asset-form__row" v-if="isInstantTrade === true">
          ...
          <div class="asset-form__price">
            <div class="asset-form__price__input">
              <input type="number" step="any" :placeholder="$t('Market.BidFormPlaceholder')" v-model="endPrice" />
              <button>
                <img :src="$static.getFileURL(`img/icon/ic-token-${getUsedCurrencySymbol()}.svg`)" />
                <span>{{ usedCurrency.symbol }}</span>
              </button>
      ...
      <div v-else-if="isSell">
        ...
        <div class="asset-form__row">
          ...
          <div class="asset-form__make-offer-nego">
            <button
              :class="$bem('asset-form__make-offer-select__option', '', isNegotiable ? ['selected'] : '')"
              @click="isNegotiable = true"
            >
              {{ $t('General.Available') }}
            </button>
            <button
              :class="$bem('asset-form__make-offer-select__option', '', !isNegotiable ? ['selected'] : '')"
              @click="isNegotiable = false"
            >
      ...
      <div v-else>
        ...
        <div class="asset-form__row">
          ...
          <div class="asset-form__price">
            <div class="asset-form__price__input">
              <input type="number" step="any" :placeholder="$t('Market.MakeOfferBuyPlaceholder')" v-model="price" />
              <button>
                <img :src="$static.getFileURL(`img/icon/ic-token-${getUsedCurrencySymbol()}.svg`)" />
                <span>{{ usedCurrency.symbol }}</span>
              </button>
            ...
            <div class="asset-form__price__balance">
              ...
              <span
                ><strong>{{ getUsedCurrencyDeposit() | addComma }}</strong> {{ usedCurrency.symbol }}
              </span>
            </div>
...
export default {
  ...
  created() {
    ...
    eventFunction2 = function (event) {
      if (!component.collectionInput && component.$refs['input'] && component.$refs['input'].contains(event.target)) {
        ...
        component.collectionInput &&
        component.$refs['inputResult'] &&
        !component.$refs['inputResult'].contains(event.target) &&
        !component.$refs['input'].contains(event.target)
      ) {
  ...
  data() {
    return {
      ...
      endPrice: '0',
      usedCurrency:
        this.$store.getters.getChainInfo.chain === 'ETHEREUM'
          ? this.$store.getters.getSupportCurrency[1]
          : this.$store.getters.getSupportCurrency[0],
      collectionKeyword: '',
      itemKeyword: '',
      selectedPeriod: {},
  ...
  computed: {
    ...mapGetters(['getSupportCurrency', 'getMaskedAddress', 'getSupportNft', 'getItemInfo', 'getChainInfo']),
  ...
  methods: {
    ...mapActions(['addAuction', 'addNormalOffer', 'setItemInfo', 'editAuction', 'editNormalOffer']),
    ...
    initModify() {
      ...
      if (getItemInfo.isExchange) {
        ...
        this.selectedPeriod =
          _.find(this.getPeriodOptions, (row) => {
    ...
    getUsedCurrencySymbol() {
      if (this.usedCurrency && this.usedCurrency.symbol && typeof this.usedCurrency.symbol === 'string')
        return this.usedCurrency.symbol.toLowerCase();
      return '';
    },
