    ...
    <section class="asset-form__body">
      ...
      <div class="asset-form__row">
        <div class="asset-form__row__label">
          ...
          <button class="asset-item__request__more" @click="$_AssetAuctionMixin_openBidStatusModal(getItemInfo)">
            <img :src="$static.getFileURL(`img/icon/ic-message-${getItemInfo.auction.bids.length > 0 ? 'black' : 'gray'}.svg`)" />
        ...
        <div class="asset-form__current-bid">
          <p>
            <strong>{{$bn.toMaxUnit(getItemInfo.auction.currentPrice, getItemInfo.currencyInfo.decimal, 4) | addComma}}</strong> {{getItemInfo.currencyInfo.symbol}}
          </p>
      ...
      <div class="asset-form__row">
        <div class="asset-form__row__label">
          ...
          <span class="asset-form__row__label__sub">
            {{$t('Market.MyBidPriceWarning', { price: ($bn.toMaxUnit(getItemInfo.auction.basePrice, getItemInfo.currencyInfo.decimal, 4 | addComma)),  token: usedCurrency.symbol })}}
          </span>
        </div>
      ...
      <div class="asset-form__row">
        ...
        <div class="asset-form__price">
          <div class="asset-form__price__timer">
            <strong>{{remainPeriod.days.setPadding(2)}}</strong>d : <strong>{{remainPeriod.hours.setPadding(2)}}</strong>h : <strong>{{remainPeriod.minutes.setPadding(2)}}</strong>m
          </div>

          <div class="asset-form__price__ended">
            {{$t('Market.EndedAt')}} <span class="divider">|</span> {{$date.formatDate(getItemInfo.auction.endTime, 'fff')}}
          </div>
        </div>
      </div>
    </section>
    ...
    <aside class="asset-form__aside">
      <asset-item-floating :note="getBidNote" :info="getItemInfo">
        <div slot="submit" class="asset-item__floating__submit">
          <button :class="$bem('common-submit-button', '', [isActiveForm ? 'primary' : 'disabled'])" @click="handleMakeBid()">
            {{$t('Market.BidNow')}}
          </button>
        </div>
      </asset-item-floating>
