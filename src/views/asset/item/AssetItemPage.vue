        ...
        <div class="asset-page__link__share">
          ...
          <button v-clipboard:copy="currentUrl" v-clipboard:success="$_GlobalValueMixin_showCopyTooltip">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="14" height="14" viewBox="0 0 14 14">
              <defs>
                <path id="c0r1rn751a" d="M6.333 2.333c.369 0 .667.299.667.667 0 .368-.298.667-.667.667h-4c-.368 0-.666.298-.666.666v7.334c0 .368.298.666.666.666h7.334c.368 0 .666-.298.666-.666v-4c0-.369.299-.667.667-.667.368 0 .667.298.667.667v4c0 1.104-.896 2-2 2H2.333c-1.104 0-2-.896-2-2V4.333c0-1.104.896-2 2-2zm6.87-1.968l.013.004c.026.01.052.02.076.032.012.005.023.011.035.018l.046.029c.035.023.068.05.098.08l-.06-.053c.016.013.032.026.046.04l.014.014.018.018.03.035c.013.015.024.03.035.047l.023.037.024.045c.008.017.016.034.022.052l.014.04c.02.062.03.128.03.197v4c0 .368-.299.667-.667.667-.368 0-.667-.299-.667-.667V2.61L6.138 8.804c-.234.234-.6.257-.86.07l-.083-.07c-.26-.26-.26-.683 0-.943l6.195-6.196H9c-.335 0-.612-.246-.66-.567L8.334 1c0-.368.299-.667.667-.667h4c.048 0 .096.005.142.016.017.003.034.008.05.013l.01.003z"/>
              </defs>
              <g fill="none" fill-rule="evenodd">
                <g>
    ...
    methods: {
      ...
      getExchangeButtons(info) {
        ...
        if (isBuy) {
          if (exchange && exchange.isBuyer) {
            ...
            let isHavingItem = Boolean(_.find(this.getMyItems, row => {
              if (this.$wallet.isSameAddress(this.$route.params.tokenAddress, row.tokenAddress) && this.$route.params.tokenId === row.tokenId) {
                return true;
              }
              return false;
            }));
      ...
      getAuctionButtons(info) {
        ...
        if(info.isEndedAuction) {
          ...
          return {
            leftButton: {
              title: (info.auction.buyerAddress && info.auction.buyerAddress !== '') ? $t('Market.BidResultSuccess') : $t('Market.BidResultFail'),
              classes: ['disabled'],
              click: () => {
