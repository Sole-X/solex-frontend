  ...
  export default {
    ...
    methods: {
      getChainDescription(chainName) {
        const ethEng = 'Ethereum is a decentralized open-source blockchain system that features its own cryptocurrency,\n' +
        ...
        const ethKor = "이더리움(Ethereum)은 자체 암호화 화폐인 '이더(Ether)'를 특징으로 하는 탈중앙화 오픈 소스 블록체인 시스템입니다. ETH는 수많은 다른 암호화의 플랫폼으로 작동하며 탈중앙화 스마트 계약도 해당 플랫폼에서 실행됩니다.";

        const klayEng = 'Kakao’s global public blockchain project Klaytn is an enterprise-grade, service-centric platform that brings user-friendly blockchain experience to millions.';
        const klayKor = '클레이튼은 카카오의 글로벌 퍼블릭 블록체인 프로젝트로, 수많은 사람에게 사용자 친화적인 블록체인 경험을 제공하는 엔터프라이즈급 서비스 중심 플랫폼입니다.';
