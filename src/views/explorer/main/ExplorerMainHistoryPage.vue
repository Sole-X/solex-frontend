        ...
        <div v-for="activity in getAllActivities.list" :key="activity.id" class="explorer-main__table__row">
          ...
          <div :class="$bem('explorer-main__table__col', '', ['token'])">
            ...
            <div class="explorer-main__table__col__token-name">
              <p><span @click="tokenClicked(activity)">{{activity.tokenInfo.name}}</span></p>
              <p v-if="activity.tokenId"><span @click="tokenClicked(activity)">#{{activity.tokenId}}</span></p>
            </div>
          </div>

          <div :class="$bem('explorer-main__table__col', '', ['price'])">
            {{$bn.toMaxUnit(activity.amount, activity.currencyInfo.decimal, 4) | addComma}} {{activity.currencyInfo.symbol}}
          </div>
          ...
          <div :class="$bem('explorer-main__table__col', '', ['user'])">
            ...
            />
            <div><span v-clipboard:copy="activity.fromAddress">{{ getItemFromAccount(activity) }}</span></div>
          </div>
          ...
          <div :class="$bem('explorer-main__table__col', '', ['user'])">
            ...
            />
            <div><span v-clipboard:copy="activity.toAddress">{{ getItemToAccount(activity) }}</span></div>
          </div>
    ...
    methods: {
      tokenClicked(item) {
        ...
        if (typeStr === 'BUY' || typeStr === 'SELL' || typeStr === 'AUCTION') {
          if (tokenAddress && tokenAddress !== '' && tokenId && tokenId !== '' && tradeId && tradeId !== '' && typeStr && typeStr !== '') {
            this.$router.push({
