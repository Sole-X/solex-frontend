...
export class AssetItemDetail extends NftInfo {
  ...
  get isEndedAuction() {
    ...
    let isTimeOver = null;
    if (this.auction)
      isTimeOver = now > $date.getTimeObject(this.auction.endTime).ts
...
export class AssetOfUser {
  ...
  get tokenInfo() {
    return _.find(Store.getters.getSupportCurrency, currency => {
  ...
  get onOrderAmount() {
    ...
    return Store.$app.$bn.addBN(lockSellAmount, lockBidAmount).addBN(lockNegoAmount).addBN(lockAuctionAmount).toString()
  }
...
export class AssetSaleOffer {
  ...
  get isAuctionSeller() {
    if (Store.$app.$wallet.isSameAddress(Store.getters.getUserInfo.address, this.ownerAddress))
      return true;
    return false;
  }
