...
export class UserActivity {
  ...
  get isProgress() {
    ...
    switch (eventType) {
      case 6:
      case 7: { // 입출금의 경우 브릿지 통과 과정은 Progress 상태로 취급
        return eventState === 'vault' || eventState === 'burn' || eventState === 'mint'
      }

      default: { // 기타는 pending, start 타입을 Progress 상태로 취급
  ...
  get currencyInfo() {
    return _.find(Store.getters.getSupportCurrency, row => {
      return Store.$app.$wallet.isSameAddress(row.addressToReserve, this.type === 'token' ? this.tokenAddress : this.currency)
    }) || {
      toFiat: () => '0'
    }
  }
