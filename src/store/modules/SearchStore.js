      ...
      if (resData.data.nftItems && resData.data.nftItems.length > 0) {
        for (const itemObj of resData.data.nftItems) {
          ...
          if (itemLowerCase.includes(wordLowerCase)) {
            ...
            const newItem = [
              item.slice(0, idxStart),
              item.slice(idxStart, idxEnd),
              item.slice(idxEnd, item.length),
      ...
      if (resData.data.nftInfos && resData.data.nftInfos.length > 0) {
        for (const collectionObj of resData.data.nftInfos) {
          ...
          if (collectionLowerCase.includes(wordLowerCase)) {
            ...
            const idxEnd = idxStart + wordLength;
            const newCollection = [
              collection.slice(0, idxStart),
              collection.slice(idxStart, idxEnd),
              collection.slice(idxEnd, collection.length),
              collectionObj.symbol,
              collectionObj.tokenAddress,
            ];
      ...
      if (resData.data.accounts && resData.data.accounts.length > 0) {
        for (const account of resData.data.accounts) {
          ...
          if (usernameLowerCase.includes(wordLowerCase)) {
            ...
            const idxEnd = idxStart + wordLength;
            const newAccount = [
              username.slice(0, idxStart),
              username.slice(idxStart, idxEnd),
              username.slice(idxEnd, username.length),
              accountAddress,
            ];
            ...
            const idxEnd = idxStart + wordLength;
            const newAccount = [
              accountAddress.slice(0, idxStart),
              accountAddress.slice(idxStart, idxEnd),
              accountAddress.slice(idxEnd, accountAddress.length),
              accountAddress,
            ];
            newItems.accounts.push(newAccount);
          }
      ...
      if (resData.data.publishers && resData.data.publishers.length > 0) {
        for (const publisher of resData.data.publishers) {
          ...
          if (publisherLowerCase.includes(wordLowerCase)) {
            ...
            const idxEnd = idxStart + wordLength;
            const newPublisher = [
              publisher.slice(0, idxStart),
              publisher.slice(idxStart, idxEnd),
              publisher.slice(idxEnd, publisher.length),
              publisher,
            ];
