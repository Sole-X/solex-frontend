...
export default {
    ...
    async initMarketMain({ getters, commit }, payload) {
      ...
      return new Promise((resolve) => {
        ...
        if (!userAddress || userAddress === '') {
          // 지갑 미연결 시
          this.$app.$http.get('getRecentViewOfAll', {}).then((res) => {
            if (res.success) {
              commit('SET_MARKET_MAIN_LIST', {
  ...
  getters: {
    ...
    getSearchQuery(state) {
      ...
      if (result.currency) {
        result.price = Object.values(
          _.find(searchFilter.price.preset, (type) => {
            return type.selected;
          }) || searchFilter.price.custom,
        ).join(',');
